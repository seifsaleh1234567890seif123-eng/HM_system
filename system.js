            ...
            ${(order.items || []).map(rawItem => {
              ...
              return `
                <div class="sys-order-item-mini-card">
                  <img src="${item.image}" class="sys-order-item-mini-img" alt="${item.name}" loading="lazy" onerror="this.src='../1شنطة دي جي مشجر مقاس 17.jpg'" />
          ...
          <div class="sys-action-btns">
            ...
            ${cleanPhone ? `
              <a href="https://wa.me/${waPhone}?text=${waMsg}" target="_blank" class="sys-icon-btn btn-wa" title="محادثة واتساب">
                <i class="fa-brands fa-whatsapp"></i> <span class="mobile-action-label">واتساب</span>
              </a>
            ` : ''}
...
function openOrderDetailsModal(orderId) {
  ...
  document.getElementById('modalCustPhone').innerHTML = `
    ...
    ${cleanPhone ? `<a href="https://wa.me/${waPhone}" target="_blank" class="cust-wa-btn" style="margin-right:8px;"><i class="fa-brands fa-whatsapp"></i> واتساب</a>` : ''}
  `;
